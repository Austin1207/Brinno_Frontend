...
exports.default = function (state, action) {
  switch (action.type) {
    ...
    //add part
    case _constants.DIRECT_CREATE_ITEM:
      state = state.merge({ sceneHistory: _export2.history.historyPush(state.sceneHistory, state.scene) });
      return _export.Item.directCreatItem(state, action.layerID, action.itemtype, action.x, action.y, action.rotation).updatedState;
