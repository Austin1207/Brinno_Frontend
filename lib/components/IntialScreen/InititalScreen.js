...
var _loadjson2 = require('./loadjson2');

var _loadjson3 = require('./loadjson3');
...
var InitialScreen = function InitialScreen(_ref) {
    ...
    if (req.status == 200) {
      ...
      if (img.complete) {
        if (img.width >= img.height) {
          projectActions.loadProject((0, _loadjson.loadimgjson)(url, jsonleft, top));
        } else {
          projectActions.loadProject((0, _loadjson2.loadimgjson2)(url, jsonleft, top));
        }
      } else {
        img.onload = function () {
          if (img.width >= img.height) {
  ...
  var Outline = function Outline(event) {
    ...
    projectActions.loadProject((0, _loadjson3.loadimgjson3)("https://example-img.s3.ap-northeast-1.amazonaws.com/small.png", jsonleft, top));
