...
import { breakIntersections, convertToSegments, compute, computeViewport, inPolygon, Position, Polygon } from 'visibility-polygon';

export default function Visibility_Polygon(_ref) {
  ...
  var segments = breakIntersections(convertToSegments(polygons));
  ...
  for (var _i2 = 0; _i2 < cameara_info.length; _i2++) {
    ...
    rendered.push(React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('path', { d: conepath })
      ),
      React.createElement(
        'g',
        { clipPath: 'url(#' + cameara_info[_i2].id + '_cone)' },
        React.createElement('polygon', { points: visibility.join(), fill: '#ff8200', 'fill-opacity': '0.15' })
      )
    ));
  }
